#!/usr/bin/env python3
"""
FastAPI Audio Processing Server with Face Recognition, AssemblyAI STT, and Groq LLM
Handles Face Recognition -> STT -> Emotion Analysis -> LLM (Groq) -> TTS pipeline
"""
...
class VoiceAssistant:
    ...
    def generate_response(self, text: str, emotion: str, user_name: str, chat_summary: Optional[str] = None) -> str:
        """Generate LLM response using Groq with emotion and context"""
        if not self.groq_client:
            return f"Hello {user_name}!. I hear what you're saying: '{text}'. I'm currently unable to generate a detailed response."
        
        try:
            # Build context-aware prompt
            emotion_context = {
                "joy": "The user seems happy and joyful. Respond with enthusiasm and positivity.",
                "sadness": "The user seems sad or down. Respond with empathy and comfort.",
                "anger": "The user seems frustrated or angry. Respond calmly and try to help resolve their concerns.",
                "fear": "The user seems worried or anxious. Respond with reassurance and support.",
                "surprise": "The user seems surprised. Respond with curiosity and engagement.",
                "disgust": "The user seems disgusted or disappointed. Respond with understanding.",
                "neutral": "The user seems neutral. Respond naturally and helpfully."
            }
            
            # Create system prompt
            system_prompt = f"""You are a helpful and emotionally intelligent voice assistant talking to {user_name}.

EMOTIONAL CONTEXT: {emotion_context.get(emotion, emotion_context['neutral'])}

CONVERSATION HISTORY: {chat_summary if chat_summary else "This is a new conversation."}
...
- Respond as if you're having a natural voice conversation"""
            ...
            return f"Hello {user_name}! I understand you're feeling {emotion}. I'm here to help, but I'm having trouble generating a response right now."
...
async def process_audio(
    ...
    """Main pipeline: Face Recognition -> STT -> Emotion Analysis -> LLM -> TTS"""
    ...
    try:
        ...
        # Get user's chat summary from database
        chat_summary = voice_assistant.db.get_user_summary(user_name)
        ...
        transcribed_text = voice_assistant.speech_to_text(temp_audio_path)
        
        # Step 2: Emotion Analysis
        logger.info("Step 2: Emotion Analysis")
        emotion_result = voice_assistant.analyze_emotion(transcribed_text)
        emotion = emotion_result["emotion"]
        
        # Step 3: LLM Response Generation with context
        logger.info("Step 3: LLM Response Generation (Groq)")
        ai_response = voice_assistant.generate_response(
            transcribed_text, emotion, user_name, chat_summary
        )
        
        # Step 4: Save conversation to database
        logger.info("Step 4: Saving conversation to database")
        voice_assistant.db.add_conversation(session_id, transcribed_text, ai_response, emotion)
        
        # Step 5: Text to Speech
        logger.info("Step 5: Text to Speech")
        audio_response = voice_assistant.text_to_speech(ai_response, emotion)
